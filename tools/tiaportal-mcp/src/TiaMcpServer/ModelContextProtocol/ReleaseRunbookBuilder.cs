...
public static class ReleaseRunbookBuilder
{
  public static JsonObject Build(JsonObject suiteRoot, JsonObject diagnostics)
  {
    ...
    var nextActions = diagnostics["recommendedNextActions"] as JsonArray ?? [];
    var observations = diagnostics["observations"]?["items"] as JsonArray ?? [];
    ...
    return new JsonObject
    {
      ...
      ["currentKnownBlocks"] = new JsonArray([
        .. observations.OfType<JsonObject>().Where(x => x["blocking"]?.GetValue<bool>() == true).Select(x =>
          new JsonObject
          {
            ["id"] = x["id"]?.ToString() ?? "",
  ...
  public static string BuildMarkdown(JsonObject runbook, string jsonPath)
  {
    ...
    foreach (var command in runbook["quickStartCommands"] as JsonArray ?? [])
    {
    ...
    foreach (var item in runbook["handoffChecklist"] as JsonArray ?? [])
    {
      md.AppendLine($"- {item}");
    }
    ...
    foreach (var item in runbook["safetyRedlines"] as JsonArray ?? [])
    {
      md.AppendLine($"- {item}");
    }
    ...
    var blocks = runbook["currentKnownBlocks"] as JsonArray ?? [];
    if (blocks.Count == 0)
    {
    ...
    foreach (var item in runbook["nextActions"] as JsonArray ?? [])
    {
      md.AppendLine($"- {item}");
    }
